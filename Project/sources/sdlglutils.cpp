...
int initFullScreen(unsigned int * width, unsigned int * height) {
	...
	if (height != NULL)
		*height = modes[0]->h;
	if (SDL_SetVideoMode(modes[0]->w, modes[0]->h,
...
int XPMFromImage(const char * imagefile, const char * XPMfile) {
	...
	image32bits = SDL_CreateRGBSurface(SDL_SWSURFACE, image->w, image->h, 32,
			rmask, gmask, bmask, amask);
	...
	for (y = 0; y < image32bits->h; y++) {
		...
		for (x = 0; x < image32bits->w; x++) {
			pixel = ((Uint32*) image32bits->pixels)[x
					+ y * image32bits->pitch / 4];
